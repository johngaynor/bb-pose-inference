...
- **`config/config.py`**: Training hyperparameters and paths
- **`config/poses.json`**: Pose definitions and metadata
- **`config/db_labels.json`**: Database labels for image-pose mapping
- **`class_mapping.json`**: Generated class mapping (created during training)
...
1. Update `config/poses.json` with new pose definitions
2. Add corresponding labels to `config/db_labels.json`
